...
public class GoldenCoinStackBlock extends Block implements Waterloggable {
    public static final IntProperty COIN_LAYERS = IntProperty.of("coin_layers", 1, 8);
    public static final BooleanProperty WATERLOGGED = Properties.WATERLOGGED;

    public GoldenCoinStackBlock(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient && player.getStackInHand(hand).isOf(ModBlocks.GOLDEN_COIN_STACK.asItem()) && hand.equals(Hand.MAIN_HAND) && state.get(COIN_LAYERS) < 8 && !player.isSneaking()) {
