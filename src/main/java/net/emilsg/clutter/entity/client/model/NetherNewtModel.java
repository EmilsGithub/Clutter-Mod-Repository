...
public class NetherNewtModel<T extends AbstractNetherNewtEntity> extends ClutterModel<T> {
	...
	@Override
	public void setAngles(AbstractNetherNewtEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.getPart().traverse().forEach(ModelPart::resetTransform);
		this.setHeadAngles(entity, netHeadYaw, headPitch, ageInTicks);

		this.animateMovement(NetherNewtAnimations.NETHER_NEWT_WALK, limbSwing, limbSwingAmount, 1.5f, 2f);
		this.updateAnimation(entity.idleAnimationState, NetherNewtAnimations.NETHER_NEWT_IDLE, ageInTicks, 1f);
	}
