...
import net.emilsg.clutter.entity.client.animation.CrimsonNewtAnimations;
...
public class CrimsonNewtModel<T extends CrimsonNewtEntity> extends ClutterModel<T> {
	...
	@Override
	public void setAngles(CrimsonNewtEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.getPart().traverse().forEach(ModelPart::resetTransform);
		this.setHeadAngles(entity, netHeadYaw, headPitch, ageInTicks);

		this.animateMovement(CrimsonNewtAnimations.CRIMSON_NEWT_WALK, limbSwing, limbSwingAmount, 1.5f, 2f);
		this.updateAnimation(entity.idleAnimationState, CrimsonNewtAnimations.CRIMSON_NEWT_IDLE, ageInTicks, 1f);
	}

	private void setHeadAngles(CrimsonNewtEntity entity, float headYaw, float headPitch, float animationProgress) {
	...
	private void updateVisibleParts(CrimsonNewtEntity crimsonNewtEntity) {
		int fungiCount = crimsonNewtEntity.getFungiCount();
